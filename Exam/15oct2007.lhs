...
> myFunction f xs ys = zipWith f xs (map (*3) $ filter (>0) ys)

3.

Haskell classes can be seen as Java interface. A data structure of a certain 
class tells you what you can do with an element of that data structure,it
supports and impelements the behaviour. For instance an element of class Eq a 
can be compared for equality and a element of class Ord can be ordered.
...
> data Bush a = Leaves [a] | Trunk (Bush [a]) deriving Show
